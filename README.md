...
| 方案 | 场景 | 保活模式 | 看门狗 | 蒸馏 | 蒸馏上限 | 召回阈值 | 召回条数 | 摘要 | 含正文 | 内存 | LLM 成本 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 🟢 **A 轻量省心** | 个人开发（默认）| `init` | 关 | 开 | 900 | 2 | 5 | 130 | 关 | ~47 MB | 正常 |
| 💰 **B 极致省钱** | 预算敏感/多 Agent | `init` | 关 | **关** | 500 | 3 | 3 | 80 | 关 | ~47 MB | **近 0** |
| ⭐ **C 高质量** | 长项目/团队 | `interval` | **开** | 开 | 1200 | 1 | 8 | 200 | 开 | ~94 MB | 高 |

> 在「设置 → 记忆」顶部配置表单里，按上表填入对应值即可切换。**当前主上已应用方案 C（高质量）**。
