  ...
  return async function handleApi(req, res) {
    ...
    switch (route) {
      ...
      case '/cards': {
        ...
        // 主库默认只显已审核；显式 status 时按状态取（pending/rejected = 供主库状态筛选）。
        const want = status === 'pending' || status === 'rejected' ? [status] : undefined
        let cards = await listCards(vaultRoot, { status: want ?? ['approved'] })
      ...
      case '/config': {
        ...
        json(res, 200, { ok: true, config: safe, revision: 0, version: env.MEMORY_ETERNAL_VERSION || '' })
        return
      }
      case '/compress': {
