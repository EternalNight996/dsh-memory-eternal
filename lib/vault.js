...
export function parseCard(text) {
  ...
  const meta = { title: '', kind: 'knowledge', tags: [], created: '', updated: '', source: '', formatVersion: 0, status: 'approved', submittedBy: '', severity: 'info', reason: '', deletedAt: '' }
  let body = text
  const m = /^---\r?\n([\s\S]*?)\r?\n---\r?\n?/.exec(text)
  if (m) {
  ...
  // 兼容旧卡：无 status 字段视为 approved（正常库）
  if (!meta.status) meta.status = 'approved'
