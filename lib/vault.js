...
/**
 * 加载召回注入提示文本（外部化，可被用户直接编辑）。
 * 优先级：
 *   1. opts.explicitFile —— 配置 recallPromptFile 显式指定的文件；
 *   2. vault 内 00-System/recall-prompt.md —— 推荐编辑位置；
 *   3. opts.bundledFile —— 包内默认模板（随发行版更新）；
 *   4. opts.fallback —— 内嵌兜底文本（保证 section 始终可注册）。
 * 文件不存在/为空/读取失败时逐级降级，永不抛错。返回最终提示文本。
 */
export async function loadRecallPrompt(root, { explicitFile = '', bundledFile = '', fallback = '' } = {}) {
  const readOne = async (p) => {
    if (!p) return ''
    try {
      const t = String(await fs.readFile(p, 'utf8')).trim()
      return t
    } catch { return '' }
  }
  const candidates = []
  if (explicitFile) candidates.push(path.resolve(explicitFile))
  if (root) candidates.push(path.join(root, '00-System', 'recall-prompt.md'))
  if (bundledFile) candidates.push(bundledFile)
  for (const p of candidates) {
    if (!p) continue
    const t = await readOne(p)
    if (t) return t
  }
...
export async function mergeCards(root, paths) {
  ...
  const r = await writeCard(root, { kind: first.meta.kind || 'knowledge', title, tags: [...tags], body: combined, source: 'merge' }, { dedup: false })
  if (!r.ok) return { ok: false, error: '写入失败' }
  for (const t of texts) { try { await deleteCard(root, t.path) } catch { /* 尽力删除 */ } }
  return { ok: true, path: r.path, merged: texts.length }
}

/** 删除卡片文件并清理空目录。 */
export async function deleteCard(root, rel) {  const target = resolveInside(root, rel)
  if (!target) throw new Error('路径越界')
