...
export async function runStandaloneCapture(vaultRoot, text, { source = '', llm, env = process.env, minChars = 120 } = {}) {
  ...
  // 自动审核：从 env 读免审配置（独立进程无 DSH settings）
  const resolveAuditStatus = (kind, sub) => {
    if ((env.MEMORY_AUDIT_MODE || 'all') === 'none') return 'approved'
    const agents = (env.MEMORY_AUDIT_EXEMPT_AGENTS || '').split(',').map((x) => x.trim()).filter(Boolean)
    const kinds = (env.MEMORY_AUDIT_EXEMPT_KINDS || '').split(',').map((x) => x.trim()).filter(Boolean)
    if (agents.includes(sub)) return 'approved'
