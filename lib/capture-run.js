...
// 读取 DSH 设置的审核偏好（写入的共享文件），使「审核设置」对不同步的独立捕获也生效。
// 优先级：共享文件 > env > 默认 all。
const AUDIT_FILE = 'memory-eternal-audit.json'
function readAuditConfig(env = process.env) {
  try {
    const file = path.join(env.DSH_HOME || path.join(os.homedir(), '.dsh'), AUDIT_FILE)
    return JSON.parse(fs.readFileSync(file, 'utf8'))
  } catch { return null }
}
export function auditConfigPath(env = process.env) {
  return path.join(env.DSH_HOME || path.join(os.homedir(), '.dsh'), AUDIT_FILE)
}
...
export async function runStandaloneCapture(vaultRoot, text, { source = '', llm, env = process.env, minChars = 120 } = {}) {
  ...
  // 自动审核：共享文件(/config)优先，其次 env，默认 all（独立进程无 DSH settings）
  const resolveAuditStatus = (kind, sub) => {
    const shared = readAuditConfig(env)
