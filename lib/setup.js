...
/** 写一个幂等的 hooks.json：events → [{command}]（cursor 风格）或 {hooks:[{type,command}]}（codex 风格）。 */
async function writeHooksJson(hooksJson, evEntries, { dryRun = false, log = () => {} } = {}) {
  ...
  for (const ev of evEntries) {
    ...
    if (arr.some((h) => JSON.stringify(h).includes('capture.mjs'))) continue
    if (!dryRun) { if (await exists(hooksJson)) await backupOnce(hooksJson); arr.push(ev.codexStyle ? { hooks: [{ type: 'command', command: ev.cmd, timeout: 60, statusMessage: 'memory: save session' }] } : { command: ev.cmd }) }
  }
...
/** 统一入口：claude 复用 setupClaude 的 SessionEnd（claude-session.js）；codex/cursor 写 capture.mjs hooks。 */
export async function connectAgentHooks(agent, opts = {}) {
  if (agent === 'claude' || agent === 'claude-code') return setupClaude({ nodePath: process.execPath, withHooks: true, log: opts.log })
  ...
  if (await exists(settingsJson)) {
    try {
      ...
      if (scfg.hooks?.SessionEnd) {
        const kept = scfg.hooks.SessionEnd.filter((h) => !(h.hooks || []).some((c) => c?.command?.includes('claude-session.js')))
